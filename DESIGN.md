...
- 音乐专属顶部横幅（`static/assets/music/banner.svg`），唱片、声波和乐谱视觉，不复用首页草坪背景。
- 歌单态和播放器态两套交互，当前使用公开试听源。

### 3.6 小说页 `/novels`

- 小说专属顶部横幅（`static/assets/novels/banner.svg`），二次元窗边草地、少女读书、书架和开卷视觉，不复用首页草坪背景。
- 书库、搜索、详情、目录和阅读器视图互斥切换。
